...
FROM cms_nav WHERE pid=$id ORDER BY sort";
     ...
     * @return StdClass|string
     */
    public function getListBreadCrumb($id)
    {
...
FROM cms_nav WHERE id=$id";
     ...
     * @return StdClass|string
     */
    public function getDetailsBreadCrumb($id)
    {
        $sql = "SELECT n2.id,n2.nav_name,n2.nav_info
FROM cms_content c JOIN cms_nav n1 ON c.nav=n1.id JOIN cms_nav n2 ON n1.pid=n2.id WHERE c.id=$id
UNION SELECT n1.id,n1.nav_name,n1.nav_info
FROM cms_content c JOIN cms_nav n1 ON c.nav=n1.id WHERE c.id=$id";
        return $this->all($sql);
    }

    /**
     * @param string $key 搜索词
     * @param int $keyType 1标题，2关键词
     * @param int $type COUNT_NEWS
     * @return mixed
     */
    public function search($key, $keyType = 1, $type = null)
    {
        switch ($keyType) {
            case 2:
        ...
        switch ($type) {
            case COUNT_NEWS:
                $sql = "SELECT count(id) count FROM cms_content" . $sql;
                return $this->one($sql)->count;
                break;
            default:
                $sql = "SELECT * FROM cms_content" . $sql . $this->limit;
                return $this->all($sql);
        }
    }
