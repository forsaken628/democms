...
<table cellspacing="0">
    ...
    {{foreach from=$newsList item=item}}
        <tr>
            <td>{{counter}}</td>
            <td><a href="../details.php?id={{$item->id}}" title="{{$item->title}}" target="_blank">{{$item->title}}</a>
            </td>
            <td>{{$item->attr}}</td>
            <td><a href="?action=show&nav={{$item->nav}}">{{$item->nav_name}}</a></td>
            <td>{{$item->count}}</td>
            <td>{{$item->date}}</td>
            <td><a href="/admin/content.php?action=update&id={{$item->id}}">修改</a> | <a href="/admin/content.php?action=delete&id={{$item->id}}"
...
<form action="?" method="get">
    <div id="page">
        <span class="me">1</span> <a href=/admin/content.php?action=show&page=2">2</a> <span class="disabled">上一页</span>
...
<form name="content" method="post" action="?action=add">
    <table cellspacing="0" class="content">
        ...
        <tr>
            <td>文档标题：<input type="text" name="title" class="text"/> <span class="red">[必填]</span> ( * 标题2-50字符之间)</td>
        </tr>
        <tr>
        ...
        <tr>
            <td><input type="submit" name="send" onclick="return checkAddContent();" value="发布文档"/> <input type="reset"
                                                                                                           value="重置"/>
            </td>
        </tr>
        <tr>
