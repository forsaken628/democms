...
class IndexAction extends Action
{
    ...
    public function searchAction()
    {
        ...
        //初始化分页
        $this->page($this->indexModel->search($clean['keyword'], $clean['type'], COUNT_NEWS), 8);
    ...
    public function detailsAction()
    {
        ...
        $details = $this->indexModel->getDetails($id);
        $details->content = htmlspecialchars_decode($details->content);
        $this->_tpl->assign('details', $details);
        $breadCrumb = $this->indexModel->getDetailsBreadCrumb($details->nav);
        $this->_tpl->assign('breadCrumb', $breadCrumb);
        $this->_tpl->assign('title', $details->title . '- 新闻系统 - CMS内容管理系统');
